...
class Setup_controller extends CI_Controller {
  ...
  function register_cost_type() {
    $this->security->get_csrf_hash();
    $item_id = ucfirst(strip_tags($this->input->post('item-id')));
    $cost_type = ucfirst(strip_tags($this->input->post('cost-type')));
    $price_cost = ucfirst(strip_tags($this->input->post('price-cost')));
    
    $this->form_validation->set_rules('item-id', 'Item ID', 'trim|required|callback_check_cost_type_exist');
    $this->form_validation->set_rules('cost-type', 'Item Description', 'trim|required|callback_check_cost_type_exist');    
    $this->form_validation->set_rules('cost-type', 'Price Cost', 'trim|required|callback_check_cost_type_exist');
