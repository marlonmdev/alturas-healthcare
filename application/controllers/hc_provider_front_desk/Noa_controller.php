  ...
  function fetch_pending_noa_requests() {
	...
	function generate_printable_noa() {
		...
		if (!$exist) {
			...
			$this->load->view('hc_provider_front_desk_panel/noa/generate_printable_noa.php');
	...
	function fetch_approved_noa_requests() {
		...
		foreach ($list as $noa) {
			...
			$custom_actions .= '<a href="' . base_url() . 'hc-provider-front-desk/noa/requested-noa/generate-printable-noa/' . $noa_id . '" data-bs-toggle="tooltip" title="View Notice of Admission
