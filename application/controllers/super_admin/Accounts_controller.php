...
class Accounts_controller extends CI_Controller {
	...
	public function fetch_all_accounts() {
		...
		foreach ($list as $account) {
			...
			if ($account['user_role'] == 'member' || $account['user_role'] === 'company-doctor') {
	...
	public function change_user_password() {
		...
		$confirm_password = strip_tags($this->input->post('confirm-password'));
		$this->form_validation->set_rules('new-password', 'New Password', 'trim|required|min_length[8]');
		$this->form_validation->set_rules('confirm-password', 'Password Confirmation', 'trim|required|matches[new-password]');
		if ($this->form_validation->run() == FALSE) {
			$response = [
				'status' => 'error',
			...
			$updated_password = $this->_hash_password($confirm_password);
			$updated_on = date("Y-m-d");
			$updated_by = $this->session->userdata('fullname');
			$this->load->model('super_admin/accounts_model');
			$updated = $this->accounts_model->db_update_user_password($user_id, $updated_password, $updated_on, $updated_by);
			if (!$updated) {
				$response = [
					'token' => $token, 
					'status' => 'save-error', 
					'message' => 'User Password Changed Failed'
				];
			}
			$response = [
				'token' => $token, 
	...
	public function change_user_account_status() {
		...
		if ($res['status'] === 'Blocked') {
			...
			$response = [
				'token' => $token, 
				'status' => 'success', 'message' => 'User Account Deactivated Successfully'
			];
		}
		echo json_encode($response);
	}
