...
class Applicants_controller extends CI_Controller {
	...
	function update_profile_pic() {
		$this->security->get_csrf_hash();
		$app_id = $this->myhash->hasher($this->input->post('app-id'), 'decrypt');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$row = $this->applicants_model->db_get_applicant_photo($app_id);
			if ($row['photo'] !== '') {
			...
			$updated = $this->applicants_model->update_profile_pic($app_id, $profile_pic);
			if (!$updated) {
				$response = [
					'status' => 'save-error', 
