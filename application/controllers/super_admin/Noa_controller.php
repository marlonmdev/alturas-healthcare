...
class Noa_controller extends CI_Controller {
	...
	function __construct() {
		parent::__construct();
		$this->load->model('super_admin/noa_model');
	...
	function fetch_all_pending_noa() {
		...
		foreach ($list as $noa) {
			...
			$view_url = base_url() . 'super-admin/noa/requested-loa/edit/' . $noa_id;
	...
	function fetch_all_approved_noa() {
		...
		foreach ($list as $noa) {
			...
			$custom_actions .= '<a href="' . base_url() . 'super-admin/noa/requested-noa/generate-printable-noa/' . $noa_id . '" data-bs-toggle="tooltip" title="Print NOA"><i class="mdi mdi-printer fs-2 text-primary"></i></a>';
	...
	function fetch_all_billed_noa() {
		...
		foreach ($list as $noa) {
			...
			$custom_actions = '<a href="JavaScript:void(0)" onclick="viewBilledNoaInfo(\'' . $noa_id . '\')" data-bs-toggle="tooltip" title="View NOA"><i class="mdi mdi-information fs-2 text-info"></i></a>';
	...
	function get_billed_noa_info() {
		$noa_id = $this->myhash->hasher($this->uri->segment(5), 'decrypt');
		$row = $this->noa_model->db_get_billed_noa_info($noa_id);
		...
		$response = [
			...
			// Full Month Date Year Format (F d Y)
			'request_date' => date("F d, Y", strtotime($row['request_date'])),
			'billed_on' => date("F d, Y", strtotime($row['billed_on'])),
			'paid_on' => date("F d, Y", strtotime($row['date_add'])),
			'work_related' => $row['work_related'],
			'percentage' => $row['percentage'],
			'req_status' => $row['status'],
			'approved_by' => $doctor_name,
	...
	function edit_noa_request() {
		...
		$this->load->view('super_admin_panel/noa/edit_noa_request');
	...
	function generate_printable_noa() {
		...
		if (!$exist) {
			...
			$this->load->view('templates/header', $data);
			$this->load->view('super_admin_panel/noa/generate_printable_noa');
			$this->load->view('templates/footer');
		}
	}
}
