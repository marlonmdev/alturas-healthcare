...
class Loa_model extends CI_Model {
  ...
  var $column_order = ['tbl_1.loa_no', 'tbl_1.first_name', 'tbl_1.loa_request_type', 'tbl_2.hp_name', null,null,null,null,null]; //set column field database for datatable orderable
  var $column_search = ['tbl_1.loa_no', 'tbl_1.emp_id', 'tbl_1.health_card_no', 'tbl_1.first_name', 'tbl_1.middle_name', 'tbl_1.last_name', 'tbl_1.suffix', 'tbl_1.loa_request_type', 'tbl_1.med_services', 'tbl_2.hp_name', 'tbl_1.request_date', 'CONCAT(tbl_1.first_name, " ",tbl_1.last_name)',   'CONCAT(tbl_1.first_name, " ",tbl_1.last_name, " ", tbl_1.suffix)', 'CONCAT(tbl_1.first_name, " ",tbl_1.middle_name, " ",tbl_1.last_name)', 'CONCAT(tbl_1.first_name, " ",tbl_1.middle_name, " ",tbl_1.last_name, " ", tbl_1.suffix)']; //set column field database for datatable searchable 
  var $order = ['tbl_1.loa_id' => 'desc']; // default order 
  ...
  // Start of server-side processing datatables
  var $table_1_pending = 'loa_requests';
  var $table_2_pending = 'healthcare_providers';
  var $table_3_pending = 'max_benefit_limits';
  var $column_order_pending = ['tbl_1.loa_no', 'tbl_1.first_name', 'tbl_1.loa_request_type', 'tbl_2.hp_name', null,null, 'tbl_1.request_date',null,null,null,null,null,null]; //set column field database for datatable orderable
  var $column_search_pending = ['tbl_1.loa_no', 'tbl_1.emp_id', 'tbl_1.health_card_no', 'tbl_1.first_name', 'tbl_1.middle_name', 'tbl_1.last_name', 'tbl_1.suffix', 'tbl_1.loa_request_type', 'tbl_1.med_services', 'tbl_2.hp_name', 'tbl_1.request_date', 'CONCAT(tbl_1.first_name, " ",tbl_1.last_name)',   'CONCAT(tbl_1.first_name, " ",tbl_1.last_name, " ", tbl_1.suffix)', 'CONCAT(tbl_1.first_name, " ",tbl_1.middle_name, " ",tbl_1.last_name)', 'CONCAT(tbl_1.first_name, " ",tbl_1.middle_name, " ",tbl_1.last_name, " ", tbl_1.suffix)']; //set column field database for datatable searchable 
  var $order_pending = ['tbl_1.loa_id' => 'desc']; // default order 

  private function _get_pending_datatables_query($status) {
    $this->db->from($this->table_1_pending . ' as tbl_1');
    $this->db->join($this->table_2_pending . ' as tbl_2', 'tbl_1.hcare_provider = tbl_2.hp_id');
    $this->db->join($this->table_3_pending . ' as tbl_3', 'tbl_1.emp_id = tbl_3.emp_id');
    $this->db->where('tbl_1.status', $status);
    $i = 0;
    ...
    // loop column 
    foreach ($this->column_search_pending as $item) {
      // if datatable send POST for search
      if ($_POST['search']['value']) {
        // first loop
        if ($i === 0) {
        ...
        if (count($this->column_search_pending) - 1 == $i) //last loop
          $this->db->group_end(); //close bracket
      }
      $i++;
    }
    ...
    // here order processing
    if (isset($_POST['order'])) {
      $this->db->order_by($this->column_order_pending[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
    } else if (isset($this->order_pending)) {
      $order = $this->order_pending;
      $this->db->order_by(key($order), $order[key($order)]);
    }
  }

  function get_pending_datatables($status) {
    $this->_get_pending_datatables_query($status);
    if ($_POST['length'] != -1)
      $this->db->limit($_POST['length'], $_POST['start']);
    $query = $this->db->get();
    return $query->result_array();
  ...
  function count_pending_filtered($status) {
    $this->_get_pending_datatables_query($status);
    $query = $this->db->get();
    return $query->num_rows();
  }

  function count_pending_all($status) {
    $this->db->from($this->table_1_pending)
             ->where('status', $status);
    return $this->db->count_all_results();
  }
  // End of server-side processing datatables
    ...
    var $column_order_billed = ['tbl_1.loa_no', 'tbl_1.first_name', 'tbl_1.loa_request_type', 'tbl_2.hp_name', null, null, 'tbl_1.request_date', null, null]; //set column field database for datatable orderable
    var $column_search_billed = ['tbl_1.loa_no', 'tbl_1.emp_id', 'tbl_1.health_card_no', 'tbl_1.first_name', 'tbl_1.middle_name', 'tbl_1.last_name', 'tbl_1.suffix', 'tbl_1.loa_request_type', 'tbl_1.med_services', 'tbl_2.hp_name', 'tbl_1.request_date', 'CONCAT(tbl_1.first_name, " ",tbl_1.last_name)',   'CONCAT(tbl_1.first_name, " ",tbl_1.last_name, " ", tbl_1.suffix)', 'CONCAT(tbl_1.first_name, " ",tbl_1.middle_name, " ",tbl_1.last_name)', 'CONCAT(tbl_1.first_name, " ",tbl_1.middle_name, " ",tbl_1.last_name, " ", tbl_1.suffix)']; //set column field database for datatable searchable 
    var $order_billed = ['tbl_1.loa_id' => 'desc']; // default order 
