...
class Count_model extends CI_Model {
  ...
  function count_all_pending_loa($emp_id) {
    $query = $this->db->get_where('loa_requests', array('status' => 'Pending', 'emp_id' => $emp_id));return $query->num_rows();
  }

  function count_all_pending_noa($emp_id) {
    $query = $this->db->get_where('noa_requests', array('status' => 'Pending', 'emp_id' => $emp_id));return $query->num_rows();
  }
