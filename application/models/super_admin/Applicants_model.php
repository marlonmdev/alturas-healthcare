...
class Applicants_model extends CI_Model {
  ...
  var $column_order = array('app_id', 'first_name', 'emp_type', 'current_status', 'business_unit', 'dept_name'); //set column field database for datatable orderable
  ...
  function db_get_applicant_photo($app_id) {
    $this->db->select('app_id, photo');
    $query = $this->db->get_where('applicants', ['app_id' => $app_id]);
    return $query->row_array();
  }

  function update_profile_pic($app_id, $profile_pic) {
    $data = [
      'photo' => $profile_pic,
    ];
    $this->db->where('app_id', $app_id);
