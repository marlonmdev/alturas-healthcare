...
<script>
  ...
  function proceedToRoute() {
    ...
    if (remainingMbl === 0) {
      window.location.href = baseUrl+"healthcare-coordinator/emergency_loa/noa_managers_key/"+emp_id;
    } else {
      window.location.href = baseUrl+"healthcare-coordinator/noa/admission_form/"+emp_id;
    }
  }
  ...
  function getMemberValues(member_id) {
    $.ajax({
      url: `${baseUrl}healthcare-coordinator/noa/member/search/${member_id}`,
      method: "GET",
